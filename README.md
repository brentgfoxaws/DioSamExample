...
```bash
DioSamExample$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
DioSamExample$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
DioSamExample$ dotnet test test/DioSamExample.Tests/DioSamExample.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name DioSamExample
```
...
## DIO Specfic Local Test

While in the folder/directory with event.json, calling the following will perform a local test

```bash
sam local invoke helloFromLambdaFunction -e event.json
```
